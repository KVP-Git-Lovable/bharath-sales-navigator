...
const DailyBreakdown: React.FC<DailyBreakdownProps> = ({ days }) => {
  const activeDays = days.filter(d => d.total > 0 || d.isPresent);
  ...
  return (
    <div className="space-y-1 animate-in slide-in-from-top-2 duration-200">
      ...
      <div className="rounded-lg border overflow-hidden">
        {/* Header */}
        <div className="grid grid-cols-5 gap-1 text-[10px] font-semibold text-muted-foreground bg-muted/50 px-3 py-2">
          <span>Date</span>
        ...
        {activeDays.map((day) => (
          <div
            key={day.date}
            className="grid grid-cols-5 gap-1 text-[11px] px-3 py-1.5 border-t border-border/50 hover:bg-muted/30 transition-colors"
          >
