-- Per-retailer payment ledger + invoice list, ported from staging.
--
-- ADDITIVE ONLY: four new functions. No table, column, policy or grant on any
-- existing object is changed, so every current screen behaves exactly as before.
--
-- The gate deliberately does NOT rely on RLS on `retailers`: this database has
-- "Authenticated users can view all retailers for analytics" with qual = true,
-- so an RLS-based gate would admit every logged-in user to every retailer's
-- payment history. The explicit predicate below is identical to staging's.
